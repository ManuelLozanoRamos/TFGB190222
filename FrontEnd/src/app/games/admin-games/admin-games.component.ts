  ...
  delete(idGame:string) : void{
    this.gameService.delete(idGame).subscribe(
      r => {
        if(r.response == 'OK'){
          this.messageService.clear();
          this.messageService.add({severity:'success', detail:'Juego eliminado con éxito.'});
          setTimeout(() => {window.location.reload();}, 3000);
        } else if(r.response == 'NOT_FOUND'){
          this.messageService.clear();
          this.messageService.add({severity:'error', detail:'No se ha podido encontrar el juego'});
        }
      },
      error => {
        this.messageService.clear();
